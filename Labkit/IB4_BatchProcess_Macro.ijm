...
//Make separate image output folders for overlay and counts
	//Overlay
OverlayFolder = output + File.separator + "IB4_Overlay";
File.makeDirectory(OverlayFolder);
	//Counts
CountsFolder = output + File.separator + "IB4_Auto_Cell_Counts";
...
//Weka IB4 Automated Positive Cell Detection
function processFile(input, filePath, output) {
	...
	classifierPath = homeDir + "/Box/Helen Lai Lab/Vihaan (HS Summer Intern)/Cell Counting Automation/IB4/IB4_Classifier.classifier";
	
	//Run Labkit
    run("Segment Image With Labkit", "segmenter_file=["+ classifierPath + "] use_gpu=false");

	IB4fileName = "IB4_"+fileName;
	rename(IB4fileName);

	//Post classification image processing
	selectImage(IB4fileName);
	postProcess();
...
saveAs("results", output + "//"+month+"."+day+"."+year+"_IB4_Cell_Count_Summary.xls");
