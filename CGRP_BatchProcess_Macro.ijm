...
//Make separate image output folders for overlay and counts
	//Overlay
OverlayFolder = output + File.separator + "CGRP_Overlay";
File.makeDirectory(OverlayFolder);
	//Counts
CountsFolder = output + File.separator + "CGRP_Auto_Cell_Counts";
...
//Weka CGRP Automated Positive Cell Detection
function processFile(input, filePath, output) {
	...
	classifierPath = homeDir + "/Box/Helen Lai Lab/Vihaan (HS Summer Intern)/Cell Counting Automation/CGRP/CGRP_Classifier.classifier";
	
	//Run Labkit
    run("Segment Image With Labkit", "segmenter_file=["+ classifierPath + "] use_gpu=false");

	CGRPfileName = "CGRP_"+fileName;
	rename(CGRPfileName);

	//Post classification image processing
	selectImage(CGRPfileName);
	postProcess();
...
saveAs("results", output + "//"+month+"."+day+"."+year+"_CGRP_Cell_Count_Summary.xls");
